...
#+author: Dylan Morgan
#+email: dbmorgan98@protonmail.com
...
4. Waste lots of time learning how to use Emacs, tweaking the configuration yourself, get frustrated, and ultimately return to using vim or VS Code
...
** TODO Completions
*** Corfu
Configure Corfu for in-buffer completion with cycling and directory preselection
...
** Evil
Do not overwrite text in the kill ring
...
** TODO Flycheck
Increase the threshold of errors before flycheck gives up
...
** LLMs
*** gptel 
Use GitHub Copilot as the GPTel backend with auto-scrolling and cursor positioning after responses
...
# [Service]
# Type=simple
# ExecStart=fish -c '/usr/bin/emacs --daemon && emacsclient -c --eval "(delete-frame)"'
...
** Tabs
Disable centaur-tabs by default but configure their appearance for when they are toggled on
...
**** Chat
Configure Copilot Chat with an org-mode frontend and extensive keybindings for code review, explanation, and generation
...
*** Indent Bars
Display visual indentation guides in programming buffers using treesitter-aware indent bars
...
*** LSP
Enable the language server for Fortran 90 buffers
...
*** Snail
Use Julia Snail as the REPL interface with history and formatting extensions
...
Add keybindings for the extensions
...
*** LSP
Enable LSP and Snail together in Julia buffers, pointing to a dedicated Julia environment for the language server
...
** LaTeX
*** Biblio
Set the default bibliography file for citation completion
...
**** Ruff
Only show LSP notifications from Ruff on errors to reduce noise
...
**** Pyright
Configure Pyright for auto-imports, completions, and basic type checking
...
*** UV
Automatically activate uv virtual environments in Python buffers and add keybindings to manage them
...
** Rust
*** Formatters and Linters
Enable format-on-save and use rust-analyzer as the LSP backend
...
* Org-mode
** Agenda
Recursively include all org files in the org directory for the agenda view
...
** Attach
Configure org attachments to use symlinks and store them in a centralised directory
...
Enable extra org special block types for richer document export
...
Automatically normalize blank lines in org buffers so that a blank line appears before a sibling entry (heading or list item) only when the previous sibling had nested content underneath it. Consecutive simple entries have no blank line between them.
...
- Use the [[https://git.tecosaur.net/tec/org-mode.git][new]] org-mode preview functionality
- Increase the rendered equation size slightly 
- Live preview to essentially replace fragtog
...
Make the background colour transparent
...
** Roam
Configure org-roam with capture templates for filing new nodes into subdirectories (personal, research, tech, work)
...
*** TODO Roam UI
Provide a graphical web UI for browsing and visualising the org-roam knowledge graph using xwidget-webkit
...
- Map src block languages to tree-sitter modes for syntax highlighting
  
- Map =json= blocks to =js-json-mode= (built-in) instead of =json-mode=.
  Doom's =+tree-sitter= flag adds =json-mode= to =major-mode-remap-alist=,
  silently redirecting it to =json-ts-mode=. Org's fontification engine uses
  traditional =font-lock=, which is incompatible with tree-sitter modes, so
  the highlighting never applies. =js-json-mode= avoids the remap and uses
  =font-lock= natively.

- Map =http= blocks to =restclient-mode= (provided by Doom's =rest= module)
  for HTTP verb/header highlighting

- Define a no-op babel execute function for JSON so =C-c C-c= doesn't error
  with "no org-babel-execute function for json"
